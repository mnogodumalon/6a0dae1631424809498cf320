...
export function KundenverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: KundenverwaltungDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'kunde_vorname': (
      <div key="kunde_vorname" className="space-y-1.5">
    ...
    'kunde_nachname': (
      <div key="kunde_nachname" className="space-y-1.5">
    ...
    'kunde_email': (
      <div key="kunde_email" className="space-y-1.5">
    ...
    'kunde_telefon': (
      <div key="kunde_telefon" className="space-y-1.5">
    ...
    'kundenkategorie': (
      <div key="kundenkategorie" className="space-y-1.5">
        <Label htmlFor="kundenkategorie">Kundenkategorie</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundenkategorie) === 'privatkunde'}
            onClick={() => setFields(f => ({ ...f, kundenkategorie: (lookupKey(f.kundenkategorie) === 'privatkunde' ? undefined : 'privatkunde') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundenkategorie) === 'privatkunde'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundenkategorie) === 'geschaeftskunde'}
            onClick={() => setFields(f => ({ ...f, kundenkategorie: (lookupKey(f.kundenkategorie) === 'geschaeftskunde' ? undefined : 'geschaeftskunde') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundenkategorie) === 'geschaeftskunde'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundenkategorie) === 'wiederverkaeufer'}
            onClick={() => setFields(f => ({ ...f, kundenkategorie: (lookupKey(f.kundenkategorie) === 'wiederverkaeufer' ? undefined : 'wiederverkaeufer') as any }))}
            className={`inline-flex items-center rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundenkategorie) === 'wiederverkaeufer'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'notizen_kunde': (
      <div key="notizen_kunde" className="space-y-1.5">
        ...
        <Textarea
          id="notizen_kunde"
          placeholder="Zahlungsart, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"kunde_vorname": "Vorname", "kunde_nachname": "Nachname", "firmenname": "Firmenname", "kunde_email": "E-Mail-Adresse", "kunde_telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Stadt", "kundenkategorie": "Kundenkategorie", "notizen_kunde": "Notizen"};
