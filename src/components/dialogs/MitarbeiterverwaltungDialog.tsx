...
export function MitarbeiterverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterverwaltungDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Mitarbeiterverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "nachname": string | null, // Nachname\n  "position": LookupValue | null, // Position (select one key: "geschaeftsfuehrung" | "drucktechniker" | "grafikdesigner" | "vertrieb" | "lager_logistik" | "buchhaltung" | "sonstiges") mapping: geschaeftsfuehrung=Geschäftsführung, drucktechniker=Drucktechniker, grafikdesigner=Grafikdesigner, vertrieb=Vertrieb, lager_logistik=Lager & Logistik, buchhaltung=Buchhaltung, sonstiges=Sonstiges\n  "email": string | null, // E-Mail-Adresse\n  "telefon": string | null, // Telefonnummer\n  "eintrittsdatum": string | null, // YYYY-MM-DD\n  "notizen_ma": string | null, // Notizen\n  "vorname": string | null, // Vorname\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
      ...
      <div key="position" className="space-y-1.5">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="drucktechniker">Drucktechniker</SelectItem>
            <SelectItem value="grafikdesigner">Grafikdesigner</SelectItem>
            <SelectItem value="vertrieb">Vertrieb</SelectItem>
            <SelectItem value="lager_logistik">Lager & Logistik</SelectItem>
    ...
    'notizen_ma': (
      <div key="notizen_ma" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"nachname": "Nachname", "position": "Position", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "eintrittsdatum": "Eintrittsdatum", "notizen_ma": "Notizen", "vorname": "Vorname"};
