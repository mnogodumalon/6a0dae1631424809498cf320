...
interface MaterialverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
...
export function MaterialverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: MaterialverwaltungDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<Materialverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'materialverwaltung');
      await onSubmit(clean as Materialverwaltung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "materialname": string | null, // Materialname\n  "materialtyp": LookupValue | null, // Materialtyp (select one key: "drucktinte" | "druckmedium" | "reinigungsmittel" | "verpackungsmaterial" | "sonstiges_material") mapping: drucktinte=Drucktinte, druckmedium=Druckmedium / Untergrund, reinigungsmittel=Reinigungsmittel, verpackungsmaterial=Verpackungsmaterial, sonstiges_material=Sonstiges\n  "einheit": LookupValue | null, // Einheit (select one key: "liter" | "kilogramm" | "stueck" | "meter" | "quadratmeter" | "rolle") mapping: liter=Liter, kilogramm=Kilogramm, stueck=Stück, meter=Meter, quadratmeter=Quadratmeter, rolle=Rolle\n  "aktueller_bestand": number | null, // Aktueller Bestand\n  "mindestbestand": number | null, // Mindestbestand\n  "preis_pro_einheit": number | null, // Preis pro Einheit (€)\n  "lieferant": string | null, // Lieferant\n  "notizen_material": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Materialverwaltung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Materialverwaltung bearbeiten' : 'Materialverwaltung hinzufügen';

  const fieldBlocks: Record<string, React.ReactNode> = {
    'materialname': (
      <div key="materialname" className="space-y-1.5">
        <Label htmlFor="materialname">Materialname</Label>
        <Input
          id="materialname"
          placeholder="z. B. Druckfarbe Cyan"
    ...
    'materialtyp': (
      <div key="materialtyp" className="space-y-1.5">
    ...
    'einheit': (
      <div key="einheit" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="einheit"><SelectValue placeholder="Einheit auswählen" /></SelectTrigger>
          <SelectContent>
    ...
    'aktueller_bestand': (
      <div key="aktueller_bestand" className="space-y-1.5">
        <Label htmlFor="aktueller_bestand">Aktueller Bestand</Label>
        <Input
          id="aktueller_bestand"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'aktueller_bestand')}
          placeholder="z. B. 25"
          value={fields.aktueller_bestand !== undefined ? fields.aktueller_bestand : (computedValues['aktueller_bestand'] ?? '')}
          onChange={e => setFields(f => ({ ...f, aktueller_bestand: clampNumberValue(formEnhancements, 'aktueller_bestand', e.target.value) }))}
        />
      </div>
    ),
    'mindestbestand': (
    ...
    'preis_pro_einheit': (
      <div key="preis_pro_einheit" className="space-y-1.5">
    ...
    'lieferant': (
      <div key="lieferant" className="space-y-1.5">
        ...
        <Input
          id="lieferant"
          placeholder="z. B. Farbwerke GmbH"
    ...
    'notizen_material': (
      <div key="notizen_material" className="space-y-1.5">
        <Label htmlFor="notizen_material">Notizen</Label>
        <Textarea
          id="notizen_material"
          placeholder="Lagerfach, Besonderheiten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"materialname": "Materialname", "materialtyp": "Materialtyp", "einheit": "Einheit", "aktueller_bestand": "Aktueller Bestand", "mindestbestand": "Mindestbestand", "preis_pro_einheit": "Preis pro Einheit (€)", "lieferant": "Lieferant", "notizen_material": "Notizen"};
  const CURRENCY_KEYS = new Set<string>(["preis_pro_einheit"]);
